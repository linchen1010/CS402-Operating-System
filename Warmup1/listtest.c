...
static void CreateTestList(My402List *pList, int num_items) {
    int i = 0;
...
static int RandomIndex(int num_values)
...
static void RandomShuffle(My402List *pList, int num_items) {
    int i = 0;
    My402List list2;
    ...
    for (i = 0; i < num_items; i++) {
        ...
        for (elem = My402ListFirst(pList); elem != NULL;
    ...
    for (elem = My402ListFirst(&list2); elem != NULL;
         elem = My402ListNext(&list2, elem)) {
        (void)My402ListAppend(pList, elem->obj);
    }
    My402ListUnlinkAll(&list2);
}

static void FindAllInList(My402List *pList, int num_items) {
    int i = 0;
...
static void PrintTestList(My402List *pList, int num_items) {
    My402ListElem *elem = NULL;
...
static void CopyTestList(My402List *pListFrom, My402List *pListTo) {
    My402ListElem *elem = NULL;

    for (elem = My402ListFirst(pListFrom); elem != NULL;
         elem = My402ListNext(pListFrom, elem)) {
        (void)My402ListAppend(pListTo, elem->obj);
    }
}

static void BubbleForward(My402List *pList, My402ListElem **pp_elem1,
                          My402ListElem **pp_elem2)
...
static void BubbleSortForwardList(My402List *pList, int num_items) {
    ...
    for (i = 0; i < num_items; i++) {
        ...
        for (elem = My402ListFirst(pList), j = 0; j < num_items - i - 1;
             elem = next_elem, j++) {
...
static void BubbleBackward(My402List *pList, My402ListElem **pp_elem1,
                           My402ListElem **pp_elem2)
...
static void BubbleSortBackwardList(My402List *pList, int num_items) {
    My402ListElem *elem = NULL;
    int i = 0;
    ...
    for (i = 0; i < num_items; i++) {
        ...
        for (elem = My402ListLast(pList), j = 0; j < num_items - i - 1;
             elem = prev_elem, j++) {
...
static void CompareTestList(My402List *pList1, My402List *pList2,
                            int num_items) {
    ...
    for (elem1 = My402ListFirst(pList1), elem2 = My402ListFirst(pList2);
         elem1 != NULL && elem2 != NULL; elem1 = My402ListNext(pList1, elem1),
        elem2 = My402ListNext(pList2, elem2), idx++) {
        int ival1 = (int)(elem1->obj);
        int ival2 = (int)(elem2->obj);

        if (ival1 != ival2) {
            fprintf(stderr,
                    "(%1d,%1d): item %1d not identical in CompareTestList().\n",
                    ival1, ival2, idx);
            exit(1);
        }
    }
